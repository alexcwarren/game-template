...
const _LEVEL_STR: Dictionary[Level, String] = {
	Level.DEBUG: "DEBUG",
	Level.INFO: "INFO",
...
static var _level: Level = Level.ERROR
...
static func log_warning(source: Object, message: String) -> void:
	_log(source, message, Time.get_datetime_string_from_system(), Level.WARNING)


static func log_error(source: Object, message: String) -> void:
	_log(source, message, Time.get_datetime_string_from_system(), Level.ERROR)


static func log_critical(source: Object, message: String) -> void:
	_log(source, message, Time.get_datetime_string_from_system(), Level.CRITICAL)


static func _log(
	source: Object, message: String, timestamp: String, level: Level
) -> void:
